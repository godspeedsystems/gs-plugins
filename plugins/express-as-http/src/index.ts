...
export default class EventSource extends GSEventSource {
  ...
  setupGoogleAuthentication(app: express.Express, googleConfig: PlainObject) {
    ...
    passport.use(
      new GoogleStrategy(
        ...
        (req, accessToken, refreshToken, params, profile, done) => {
          ...
          const state = this.parseJsonIfPossible(
            stateParam ? decodeURIComponent(stateParam.toString()) : null,
          );

          return done(null, { ...profile, $state: state });
        },
      ),
    );
    ...
    app.get(authRoute, (req, ...args) => {
      const queryState = req.query?.state;
      const state = queryState ? encodeURIComponent(queryState.toString()) : "";

      passport.authenticate("google", { state })(req, ...args);
    });
    app.get(
      callbackRoute,
  ...
  setupLinkedInAuthentication(
    ...
    app.get(authRoute, (req, ...args) => {
      ...
      passport.authenticate("linkedin", { state })(req, ...args);
    });
  ...
  setupGithubAuthentication(app: express.Express, githubConfig: PlainObject) {
    ...
    passport.use(
      new GithubStrategy(
        ...
        (req, accessToken, refreshToken, result, profile, done) => {
          const stateParam = req.query?.state;
          const state = this.parseJsonIfPossible(
            stateParam ? decodeURIComponent(stateParam.toString()) : null,
          );
  ...
  setupMetrics(app: express.Express) {
    ...
    const validateBuckets = (buckets: any, name: string) => {
      ...
      if (
        ...
        !buckets.every((b) => typeof b === "number" && !isNaN(b))
      ) {
  ...
  private parseJsonIfPossible(value: any) {
    ...
    try {
      ...
      // Only return parsed if it's an object, array, number, boolean, or null
      if (
